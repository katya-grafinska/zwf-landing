...
<div class="site-info" <?php if($event_settings['event-img-upload-footer-image'] !=''){?>style="background-image:url('<?php echo esc_url($event_settings['event-img-upload-footer-image']); ?>');" <?php } ?>>
	...
	<?php
		...
		dynamic_sidebar( 'event_footer_options' );
			...
			<a title="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>" href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php echo get_bloginfo( 'name', 'display' ); ?></a> |
            <?php esc_html_e('Designed by:','event'); ?> <a title="<?php echo esc_attr__( 'Theme Freesia', 'event' ); ?>" target="_blank" href="<?php echo esc_url( 'https://www.facebook.com/katyagraph' ); ?>"><?php esc_html_e('Kateryna Hrafinska','event');?></a> |
